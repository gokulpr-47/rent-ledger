...
exports.closeRentalService =
  exports.getAllCustomerRentalsService =
  exports.getCustomerRentalItemsService =
  exports.updateRentalItemPriceService =
  exports.updateReturnedTimeService =
  exports.addItemsToOpenRentalService =
    void 0;
...
const addItemsToOpenRentalService = async ({ customerId, items }) => {
  ...
  try {
    ...
    // 🔹 Insert rental items
    const createdItems =
      await rentalItem_model_1.RentalItem.insertMany(rentalItemsToInsert);
...
const updateRentalItemPriceService = async (rentalItemId, newPrice) => {
  ...
  // update the stored total directly
  item.total = newPrice;
  await item.save();

  // recalc parent rental totals for any returned items
  const rentalItems = await rentalItem_model_1.RentalItem.find({
    rental: item.rental,
    returnedTime: { $ne: null },
  });
...
const getCustomerRentalItemsService = async (customerId) => {
  ...
  const rental = await rental_model_1.Rental.findOne({
    customer: customerId,
    status: "OPEN",
  });
  if (!rental) {
  ...
  // recalc totals from items in case rental document is stale
  ...
  // determine which items have been covered by previous payments
  const paidItemIds = new Set();
  payments.forEach((p) => {
    if (p.items && Array.isArray(p.items)) {
      p.items.forEach((id) => paidItemIds.add(id.toString()));
    }
  });
  const itemsWithPaid = items.map((i) => ({
    ...i.toObject(),
    paid: paidItemIds.has(i._id.toString()),
  }));
...
const getAllCustomerRentalsService = async (customerId) => {
  ...
  for (const rental of rentals) {
    ...
    // recalc totals from items in case rental doc is stale
    const returnedItemsTotal = items
      .filter((i) => i.returnedTime)
      .reduce((sum, i) => sum + i.total, 0);
    if (rental.totalAmount !== returnedItemsTotal) {
